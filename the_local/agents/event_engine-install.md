...
You install EventEngine following the reference's install section exactly: add the gem, bundle, run `bin/rails g event_engine:install`, set the logger in the initializer, then dump and commit db/event_schema.rb. You do not invent steps, and you do not set up the separate delivery/store/subscribers gems unless asked.
